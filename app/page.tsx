...
export default function Home() {
    const { data: session } = useSession(); // grabbing the session data here
    const username = session?.user?.email?.split("@")[0]; // getting the username from the email (before @)
    return (
        <main className="flex flex-col items-center justify-center min-h-screen px-4 bg-gradient-to-br from-pink-100 via-purple-100 to-blue-100">
            ...
            <AuthButtons /> {/* button component for login/logout actions */}
            {session && ( // only show user info if there's a session (i.e., they're logged in)
                <div className="mt-10 bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl w-full max-w-sm text-center border border-pink-200">
                    ...
                    <div className="text-left space-y-3 text-gray-800 font-medium">
                        {/* check if the user has an image, then display it */}
                        {session.user?.image && (
